...
def build_hcitem(csvRow):
	...
	r = hypercat.Resource("ferry ports {:s}: live data".format(csvRow[7]),  "application/json")
...
def parse_csv():
	...
	h = hypercat.Hypercat("{:s} ferry catalogue".format(PROVIDER_NAME))
	...
	with open(DBDUMP_PATH, 'rb') as csvfile:
		...
		for i, row in enumerate(dbreader):
			...
			r = build_hcitem(row)
			h.addItem(r, 'http://fcc.transportapi.com/v3/uk/ferry/stop/{:s}/live.json'.format(row[1]))
...
def generate_hypercat_file():
	...
	file_name = 'output/ferry-live.json' # the file name
