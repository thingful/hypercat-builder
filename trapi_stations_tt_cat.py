...
def build_hcitem(csvRow):
	...
	r = hypercat.Resource("{:s} station: timetable data".format(csvRow[3]),  "application/json")
...
def parse_csv():
	...
	with open(DBDUMP_PATH, 'rb') as csvfile:
		...
		for i, row in enumerate(dbreader):
			...
			r = build_hcitem(row)
			h.addItem(r, 'http://fcc.transportapi.com/v3/uk/train/station/{:s}/timetable.json'.format(row[5]))
...
def generate_hypercat_file():
	...
	file_name = 'output/stations-timetable.json' # the file name
