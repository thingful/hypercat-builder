	...
	def test_build_hcitem_station(self):
		...
		with open('test/test-station.csv') as data:
			...
			for i, row in enumerate(reader):
				...
				expected.addRelation('http://www.w3.org/2003/01/geo/wgs84_pos#lat', '')
				expected.addRelation('http://www.w3.org/2003/01/geo/wgs84_pos#long', '')
...
class buildIndexTest(unittest.TestCase):
	...
	@mock.patch('hypercat_builder.os.listdir')
	def test_index(self, mock_dir):
		hc = HypercatBuilder('input_dir', 'output_dir', 'http://transportapi.com')

		# set up mock 
		mock_dir.return_value = ['ferry', 'train']
